...
function M.buildStatusUI()
    ...
    frame.Parent = gui
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 3) -- QUASE QUADRADO
...
-- ============================================================
-- MOBILE BUTTONS (trascinabili singolarmente) - UNCHANGED
-- ============================================================
...
function M.buildMobileButtons()
    ...
    for i, def in ipairs(btnDefs) do
        ...
        if M.circleButtonsEnabled then
            ...
            corner.CornerRadius = UDim.new(0, 3) -- QUASE QUADRADO
        end
        ...
        if mobImgId > 0 then
            ...
            bgc.CornerRadius = M.circleButtonsEnabled and UDim.new(1, 0) or UDim.new(0, 3) -- QUASE QUADRADO
...
local CHERRY_THEMES = {
    -- PHANTOM DUELS theme: Black, White, Dark Grey, Purple Accent
    Default  = { Accent=Color3.fromRGB(180,80,255), AccentDim=Color3.fromRGB(130,60,200), Bg=Color3.fromRGB(0,0,0),     Row=Color3.fromRGB(8,8,12) },
}
if M._savedTheme and CHERRY_THEMES[M._savedTheme] then
    CherryConfig.Theme = M._savedTheme
end
M.colorScheme = CherryConfig.Theme
M.customBgId = 6311243701 -- Dark Anime Girl
M.customBgOpacity = 0.25
M.mobBtnBgId = 0
M.BG_IMAGE_IDS = {
    6311243701,
}
...
local UI_ACCENT       = Color3.fromRGB(180, 80, 255)
local UI_ACCENT_DIM   = Color3.fromRGB(130, 60, 200)
local UI_BG_DARK      = Color3.fromRGB(0,0,0)
local UI_ROW_BG       = Color3.fromRGB(0,0,0)
local UI_CARD_STROKE  = Color3.fromRGB(60, 60, 70)
...
local function uiCardStyle(f)
    local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, 3); c.Parent = f -- QUASE QUADRADO
    local s = Instance.new("UIStroke"); s.Thickness = 1; s.Color = UI_CARD_STROKE or Color3.fromRGB(60, 60, 70); s.Transparency = 0.45; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Parent = f
...
local function uiSmallBtn(p)
    ...
    local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0,p.CR or 3); c.Parent = b -- QUASE QUADRADO
    return b
end
...
local function uiInputRow(parent, label, def, hidden)
    ...
    Instance.new("UICorner",bx).CornerRadius = UDim.new(0,3) -- QUASE QUADRADO
    return r, bx
end
...
local function uiToggleRow(parent, label, on, callback)
    ...
    Instance.new("UICorner",tb).CornerRadius = UDim.new(0,3) -- QUASE QUADRADO
...
local function uiChoiceRow(parent, label, options, defaultIndex, callback)
    ...
    local la = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-174,0,8), Size=UDim2.new(0,29,0,27), Text="<", Col=UI_TEXT_PRIMARY, TS=13, CR=3}) -- QUASE QUADRADO
    ...
    Instance.new("UICorner",vl).CornerRadius=UDim.new(0,3) -- QUASE QUADRADO
    local ra = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-35,0,8), Size=UDim2.new(0,29,0,27), Text=">", Col=UI_TEXT_PRIMARY, TS=13, CR=3}) -- QUASE QUADRADO
...
local function uiExpandToggleRow(parent, label, on, options, defaultIndex, onToggle, onOption)
    ...
    arrow.Parent = r
    Instance.new("UICorner", arrow).CornerRadius = UDim.new(0, 3) -- QUASE QUADRADO
    ...
    for i, opt in ipairs(options) do
        ...
        b.Parent = optParent
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 3) -- QUASE QUADRADO
        if useScroll then
...
function M.openImagePicker(kind)
    ...
    if isBg then
        ...
        opLbl.TextSize = 10
        opLbl.TextXAlignment = Enum.TextXAlignment.Left        opLbl.ZIndex = 3
        opLbl.Parent = panel
...
function M.buildGui()
    ...
    frameCorner.CornerRadius = UDim.new(0, 3) -- QUASE QUADRADO
    ...
    Instance.new("UICorner",MinBtn).CornerRadius = UDim.new(0,3) -- QUASE QUADRADO
    ...
    Instance.new("UICorner",lockButton).CornerRadius = UDim.new(0,3) -- QUASE QUADRADO
    ...
    MinPill.AnchorPoint = Vector2.new(0, 0) -- Anchor to top-left
    -- Top-left position
    MinPill.Position = UDim2.new(0, 12, 0, 12)
    ...
    Instance.new("UICorner",MinPill).CornerRadius=UDim.new(0, 3) -- QUASE QUADRADO
    do
    ...
    do
        local l=Instance.new("TextLabel"); l.Size=UDim2.new(1,0,1,0); l.BackgroundTransparency=1; l.Text="PHANTOM DUELS"; l.TextColor3=UI_ACCENT; l.TextSize=11; l.Font=Enum.Font.GothamBlack; l.Parent=MinPill
        ...
        MinPill.InputBegan:Connect(function(i)
            if M.uiLocked then return end
            if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
                drag=true; dStart=i.Position; sPos=MinPill.Position
                i.Changed:Connect(function() if i.UserInputState==Enum.UserInputState.End then drag=false end end)
            end
        end)
        MinPill.InputChanged:Connect(function(i)
            if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then
                if drag then
                    local d=i.Position-dStart
                    MinPill.Position=UDim2.new(sPos.X.Scale,sPos.X.Offset+d.X,sPos.Y.Scale,sPos.Y.Offset+d.Y)
                end
            end
        end)
        UIS.InputChanged:Connect(function(i)
    ...
    local function uiKeybindRow(parent, label, kbEntry)
        ...
        local btn = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-150,0.5,-12), Size=UDim2.new(0,140,0,25),
            ...
            Col=UI_TEXT_DIM, TS=10, CR=3}) -- QUASE QUADRADO
    ...
    do
        ...
        local carryBtn = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-100,0.5,-13), Size=UDim2.new(0,88,0,26),
            Text=M.carrySpeedActive and "Carry On" or "Carry Off", Col=UI_TEXT_PRIMARY, TS=12, CR=3, SC=UI_ACCENT, STr=0.3}) -- QUASE QUADRADO
    ...
    do
        ...
        local modeBtn = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-100,0.5,-13), Size=UDim2.new(0,88,0,26),
            Text=M.laggerModeEnabled and "Lag On" or "Lag Off", Col=UI_TEXT_PRIMARY, TS=12, CR=3, SC=UI_ACCENT, STr=0.3}) -- QUASE QUADRADO
    ...
    do
        ...
        local modeBtn = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-100,0.5,-13), Size=UDim2.new(0,88,0,26),
            Text=M.laggerCarryActive and "L.Carry On" or "L.Carry Off", Col=UI_TEXT_PRIMARY, TS=12, CR=3, SC=UI_ACCENT, STr=0.3}) -- QUASE QUADRADO
        modeBtn.MouseButton1Click:Connect(function()
            M.toggleLaggerCarry()
    ...
    do
        ...
        box.Parent = r
        Instance.new("UICorner", box).CornerRadius = UDim.new(0, 3) -- QUASE QUADRADO
        ...
        local minus = uiSmallBtn({
            ...
            Text = "-", Col = UI_TEXT_PRIMARY, TS = 14, CR = 3 -- QUASE QUADRADO
        })
    ...
    do
        ...
        local sBtn = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-80,0.5,-13), Size=UDim2.new(0,68,0,26),
            Text="SAVE", Col=Color3.fromRGB(200,200,200), TS=12, CR=3, SC=Color3.fromRGB(40,40,40), STr=0.2}) -- QUASE QUADRADO
    ...
    do
        ...
        local rBtn = uiSmallBtn({Parent=r, Pos=UDim2.new(1,-80,0.5,-13), Size=UDim2.new(0,68,0,26),
            Text="RESET", Col=Color3.fromRGB(200,200,200), TS=12, CR=3, SC=Color3.fromRGB(40,40,40), STr=0.2}) -- QUASE QUADRADO
